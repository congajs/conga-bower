...
Bower.prototype = {
	...
	onServerBoot: function(event, next){
		...
		try {
			...
			if (!this.compareChecksum(container)){
				...
				container.get('logger').info('[conga-bower] - nothing to install');
				next();
			}
	...
	runBower: function(container, next){
		...
		container.get('logger').info('[conga-bower] - running bower dependency installation');
		...
		cmd.on('close', function(data){
			...
			container.get('logger').info('[conga-bower] - finished installing dependencies');
			next();
		});
	},
	...
	createBowerJson: function(container){
		...
		if (config.resolutions) {
			json.resolutions = config.resolutions;
		}
